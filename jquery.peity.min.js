...
l.isFunction(a[0])?a.map(function(a){return l.isFunction(a)?a:function(c,e){return a[e%a.length]}}):function(b,c){return a[c%a.length]}:a};J.draw=function(){w.graphers[this.type].call(this,this.opt)};J.prepareCanvas=function(a,b){var c=this.canvas;c||(this.canvas=c=l("<canvas class='peity'>")[0],this.context=c.getContext("2d"),this.$el.hide().removeData("mouse").after(c));c.width=a;c.height=b;l(c).css({height:b,width:a}).data("peity",this).off();return c};J.values=function(){var a=arguments,b=this.$el.text().split(a[0]);
return 1===a.length?u.parseFloats(b):b.map(function(b){return u.parseFloats(b.split(a[1]))})};var u={defaultText:{color:"#000",size:13,formatter:function(a){return a}},labelLeveler:function(a){return p.max.apply(this,a.map(function(a){return a.replace(/[^ ]/g,"").length}))+1},stringifier:function(a,b){return b?a.map(function(a){return b(a)+""}):a.map(function(a){return a+""})},parseFloats:function(a){return a.map(function(a){return parseFloat(a)})},extractor:function(a){return function(b){return b[a]}},
